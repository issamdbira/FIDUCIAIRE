...
export default function TesteurTXT() {
  ...
  return (
    <div className="max-w-3xl mx-auto py-8 px-4">
      ...
      <Card
        className={`p-8 rounded-lg shadow-sm border-2 border-dashed text-center cursor-pointer ${dragging ? "border-primary bg-primary/5" : "border-border"}`}
      ...
      >
        <label className="flex flex-col items-center gap-2 cursor-pointer">
          ...
          <span className="text-muted-foreground">Glissez-déposez vos fichiers TXT ici, ou cliquez pour sélectionner</span>
          <input
            type="file"
            multiple
            accept=".txt,*"
      ...
      {resultats.length > 0 && (
        <div className="space-y-4 mt-6">
          {resultats.map((r, i) => (
            <Card key={i} className="p-6 rounded-lg shadow-sm border border-border bg-card">
              ...
              {r.valid ? (
                <>
                  ...
                  <details className="mt-3">
                    <summary className="text-sm text-primary cursor-pointer">Détail ligne par ligne</summary>
                        ...
                        <tbody>
                          {r.employees.map((e, k) => (
                            <tr key={k} className="border-b border-border">
                              ...
                              <td className={`py-1 pr-2 font-mono ${e.zoneOk ? "text-success" : "text-destructive"}`}>{e.zoneVierge}</td>
                            </tr>
                          ))}
                        </tbody>
                      </table>
                ...
                <p className="text-sm text-destructive mt-1">❌ {r.error || "Fichier invalide ou vide"}</p>
              )}
            </Card>
          ))}
